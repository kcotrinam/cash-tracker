...
- `prisma` — future Prisma schema and migrations (not introduced in the foundation phase).
- `infra/docker/docker-compose.yml` — local PostgreSQL development environment.
- `AGENTS.md` — this file.
...
- Backend: NestJS, TypeScript, PostgreSQL, Swagger/OpenAPI for API spec. Prisma is added only in the later data phase.
...
- Once Prisma is introduced, make every DB schema change through a Prisma migration and commit it. Do not edit production schema manually.
...
- Types: Generate API client types from OpenAPI; place generator in `packages/` and commit generated types to avoid friction in PRs. Do not manually duplicate API DTOs in `packages/api-client`.
...
- Migrations: once Prisma exists, use Prisma migrations for every DB change. Run locally and in CI. Include a migration in any PR that touches the schema.
...
- Once Prisma is introduced, always run migrations locally and include them in PRs. If unsure, ask a human reviewer to confirm migration strategy for production.
