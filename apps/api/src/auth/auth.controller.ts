...
export class AuthController {
  ...
  private set(
    ...
    const secure = this.config.get('nodeEnv') === 'production';
    const domain = this.config.get('cookieDomain');
    response.cookie('cashtracker_access', tokens.accessToken, {
      httpOnly: true,
      secure,
      sameSite: 'lax',
      maxAge: 900000,
    ...
    response.cookie('cashtracker_refresh', tokens.refreshToken, {
      httpOnly: true,
      secure,
      sameSite: 'lax',
  ...
  @Post('change-password') @HttpCode(200) @ApiCookieAuth() @ApiBearerAuth() @UseGuards(AccessTokenGuard) @Throttle({ default: { limit: 5, ttl: 60000 } }) async changePassword(@CurrentUser() user: { id: string }, @Body() dto: ChangePasswordDto, @Res({ passthrough: true }) res: Response) { const result = await this.auth.changePassword(user.id, dto); this.set(res, result); return {}; }
  @Get('me') @ApiCookieAuth() @ApiBearerAuth() @UseGuards(AccessTokenGuard) async me(@CurrentUser() user: { id: string }) { return { user: await this.auth.me(user.id) }; }
}
